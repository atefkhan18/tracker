...
for n in range (0-numRange, numRange):
  ...
  if caseNum not in visited:
    ...
    # print br.form
    br.form["appReceiptNum"] = 'WAC' + caseNum
    ...
    for status in soup.findAll('div', {'class': 'rows text-center'}):
      if all (keyWord in status.text for keyWord in [formType]):
        print(status.text)
        receiptNum = re.search('WAC(\d+)', status.text).group(1)
...
print('*********************************')
print('For ' + str(2*numRange) + ' neighbors of WAC' + str(myCaseNum) +', we found the following statistics: ')
